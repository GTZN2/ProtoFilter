...
#Prototypes, distance
Proto_PER_wrongclass_right = [[9.26495726, 7.04273504]]
Proto_PER_wrongclass_right_disp = [0.9999918741644774]
Proto_PER_wrongclass_right_dis2pro = [3.038860023683693]
Proto_PER_wrongclass_right_ni = [[[0.83874637, 0.01671245], [0.01671245, 0.09535373]]]

Proto_PER_wrongclass_wrong = [[7.88050314, 7.27044025]]
Proto_PER_wrongclass_wrong_disp = [0.9999935824700837]
Proto_PER_wrongclass_wrong_dis2pro = [2.908394085446245]
Proto_PER_wrongclass_wrong_ni = [[[ 0.22379063, -0.02007938], [-0.02007938,  0.13642356]]]

Proto_PER_missTarget_right = [[0,  7.5]]
Proto_PER_missTarget_right_disp = [0.9977621514787236]

Proto_PER_missTarget_wrong = [[0,   5.08377167]]
Proto_PER_missTarget_wrong_disp = [0.9998508197697431]

Proto_PER_missMISC_right = [[9.8861465, 0]]
Proto_PER_missMISC_right_disp = [0.5596191869899888]
Proto_PER_missMISC_right_dis2pro = [0.21175301229258855]

Proto_PER_missMISC_wrong = [[8.56235294, 0]]
Proto_PER_missMISC_wrong_disp = [0.9780883031022612]
                    ...
                    for i in range(len(predict_PER_entity_list)):
                        ...
                        if predict_PER_entity_list[i] in predict_MISC_entity_list:
                            ...
                            if PER_compareDistance_for_wrongclass_to_remove(
                                    [predict_PER_rating_list[i], predict_MISC_rating_list[MISC_index]], llm_index, a):
                                final_PER_prediction_list.remove(predict_PER_entity_list[i])

                        elif PER_compareDistance_for_missMISC([predict_PER_rating_list[i], 0], llm_index, b):
                            final_PER_prediction_list.remove(predict_PER_entity_list[i])
                                            ...
                                            if len(res1_list[k].split()) > 1:
                                                if len(cell_entity) > indexes[j] + 1:
                                                    if cell_class[indexes[j] + 1] == 'I-PER':
                                                        if cell_entity[indexes[j] + 1].lower() == res1_list[k].split()[1].lower():
                                ...
                                for j in res1_list:
                                    if j not in checked_entity_in_prediction:
                                        if cell_entity[indexes_inside[inside_entity_index]].lower() == j.split()[0].lower():
                                            ...
                                            if cell_entity[indexes_inside[inside_entity_index] - 1].lower() not in [s.split()[0].lower() for s in res1_list]:
                                                PER_miss_by_lmm -= 1
                                            break
